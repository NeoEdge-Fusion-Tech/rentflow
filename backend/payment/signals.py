    ...
    if created:
        ...
        if invoice:
            ...
            if instance.payment_status == "paid" and invoice.status != "paid":
                ...
                invoice.status = (
                    "issued"  # revert to issued if booking is no longer fully paid
                )
                updated_fields.append("status")
...
def create_payment_on_booking_paid(sender, instance, **kwargs):
    ...
    if instance.payment_status == "paid":
        ...
        existing_paid_total = (
            Payment.objects.filter(booking=instance, status="completed").aggregate(
                total=Sum("amount")
            )["total"]
            or 0
        )
...
def update_booking_paid_amount(sender, instance, **kwargs):
    ...
    if instance.booking:
        ...
        total_paid = (
            Payment.objects.filter(booking=booking, status="completed").aggregate(
                total=Sum("amount")
            )["total"]
            or 0
        )
    ...
    if instance.status == "paid":
        ...
        existing_paid_total = (
            Payment.objects.filter(
                invoice_record=instance, status="completed"
