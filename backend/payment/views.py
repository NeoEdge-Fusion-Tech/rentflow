...
from .utils import (
    ...
    initialize_paystack_transaction_for_invoice,
    initialize_paystack_transaction_for_subscription,
...
class PaymentViewSet(TenantIsolationMixin, viewsets.ModelViewSet):
    ...
    def create_payment_link(self, request):
        ...
        try:
            booking = Booking.objects.get(
                pk=booking_id, organization=request.user.organization
            )
    ...
    def create_subscription_link(self, request):
        ...
        try:
            ...
            if not org_id:
                return Response(
                    {"error": "No organization attached to user."}, status=400
                )

            paystack_data = initialize_paystack_transaction_for_subscription(
                email, amount, org_id, plan_name
            )
...
class SuperAdminRevenueAPIView(APIView):
    ...
    def get(self, request):
        ...
        # Breakdown by plan
        plan_breakdown = list(
            Subscription.objects.values("plan_name").annotate(
                count=Count("subscription_id")
            )
        )

        recent_payments = SubscriptionPaymentSerializer(
            SubscriptionPayment.objects.filter(status="successful").order_by(
                "-payment_date"
            )[:10],
            many=True,
        ).data
...
class OrganizationSubscriptionAPIView(APIView):
    ...
    def get(self, request):
        if not hasattr(request.user, "organization") or not request.user.organization:
            return Response(
                {"error": "No organization attached"},
...
class InvoiceViewSet(TenantIsolationMixin, viewsets.ModelViewSet):
            ...
            if plan:
                if not plan.has_invoice:
                    raise PermissionDenied(
                        "Your current subscription plan does not include the Invoicing module. Please upgrade to continue."
                    )
                ...
                if not serializer.validated_data.get("booking"):
                    ...
                    if quota != -1:
                        ...
                        if current_month_invoices >= quota:
                            raise PermissionDenied(
                                f"You have reached your limit of {quota} standalone invoices per month. Please upgrade your plan or contact support to continue."
                            )
    ...
    def perform_destroy(self, instance):
        ...
        if instance.status == "cancelled":
            if self.request.user.role != "admin":
                raise PermissionDenied(
                    "Only organization admins can permanently delete invoices from the trash."
                )
    ...
    def empty_trash(self, request):
        ...
        return Response(
            {"message": f"{count} invoices permanently deleted."}, status=200
        )
    ...
    def record_payment(self, request, pk=None):
        ...
        if amount > invoice.amount_left:
            return Response(
                {
                    "error": f"Amount exceeds remaining balance of {invoice.amount_left}."
                },
                status=400,
            )
    ...
    def next_number(self, request):
        ...
        last_invoice = (
            Invoice.objects.filter(organization=user.organization)
            .order_by("-created_at", "-invoice_id")
            .first()
        )
...
class QuotationViewSet(TenantIsolationMixin, viewsets.ModelViewSet):
    ...
    def perform_destroy(self, instance):
        ...
        if instance.status == "converted":
            raise ValidationError(
                "Cannot delete a quotation that has already been converted to an invoice."
            )

        if instance.status == "cancelled":
            if self.request.user.role != "admin":
                raise PermissionDenied(
                    "Only organization admins can permanently delete quotations from the trash."
                )
            super().perform_destroy(instance)
        else:
            instance.status = "cancelled"
    ...
    def empty_trash(self, request):
        ...
        org = request.user.organization
        count, _ = Quotation.objects.filter(
            organization=org, status="cancelled"
        ).delete()
        return Response(
            {"message": f"{count} quotations permanently deleted."}, status=200
        )
    ...
    def next_number(self, request):
        ...
        last_quotation = (
            Quotation.objects.filter(organization=user.organization)
            .order_by("-created_at", "-quotation_id")
            .first()
        )
    ...
    def convert_to_invoice(self, request, pk=None):
        ...
        return Response(
            InvoiceSerializer(invoice, context={"request": request}).data, status=201
        )
    ...
    def generate(self, request):
        ...
        try:
            payment = Payment.objects.get(
                pk=payment_id, organization=request.user.organization
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if event == "charge.success":
            ...
            if data["data"].get("metadata", {}).get("source") == "subscription_upgrade":
                ...
                try:
                    ...
                    subscription, _ = Subscription.objects.get_or_create(
                        organization=org, defaults={"plan_name": plan_name}
                    )
                    ...
                    logger.info(
                        f"Subscription upgraded for org {org.name} to {plan_name}"
                    )
                    return Response(
                        {"status": "success", "source": "subscription_upgrade"},
                        status=200,
                    )
                except Organization.DoesNotExist:
                    logger.error(
                        f"Organization {org_id} not found for subscription payment {reference}"
                    )
            ...
            if invoice and invoice.status != "paid":
                ...
                if invoice.booking:
                    booking = invoice.booking
                    booking.amount_paid = float(booking.amount_paid) + float(
                        amount_paid
                    )
                    if booking.amount_paid >= booking.total_amount:
                        booking.payment_status = "paid"
                ...
                logger.info(
                    f"Invoice {invoice.invoice_number} received payment via Paystack ref {reference}"
                )
...
class GeneralExpenseViewSet(TenantIsolationMixin, viewsets.ModelViewSet):
    ...
    def stats(self, request):
        ...
        current_month_amount = (
            queryset.filter(date__gte=current_month).aggregate(total=Sum("amount"))[
                "total"
            ]
            or 0
        )
