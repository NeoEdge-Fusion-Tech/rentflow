...
class TenantIsolationMixin:
    ...
    def get_queryset(self):
        ...
        if model_name == "BookingItemUnit":
            return qs.filter(
                booking_item__booking__organization_id=user.organization_id
            )
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name, field in self.fields.items():
            if (
                ...
                and isinstance(field.child_relation, serializers.PrimaryKeyRelatedField)
                ...
                if hasattr(field.child_relation.queryset.model, "organization"):
                    field.child_relation.queryset = (
                        field.child_relation.queryset.filter(
                            organization_id=user.organization_id
                        )
                    )
                elif field.child_relation.queryset.model == Organization:
                    field.child_relation.queryset = (
                        field.child_relation.queryset.filter(id=user.organization_id)
                    )
