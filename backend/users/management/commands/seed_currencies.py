...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for data in currencies:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f"Created currency {data['code']}")
                )
                count += 1
            else:
                self.stdout.write(
                    self.style.WARNING(f"Currency {data['code']} already exists")
                )

        self.stdout.write(
            self.style.SUCCESS(f"Successfully seeded {count} currencies!")
        )
