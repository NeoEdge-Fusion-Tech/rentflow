...
def extract_imports_from_project(project_path):
    ...
    third_party_imports = {
        imp for imp in extractor.imports if imp not in stdlib and imp != ""
    }

    # Filter out internal local modules (heuristic: check if there's a folder/file matching the name)
    external_imports = set()
    for imp in third_party_imports:
        if not os.path.exists(os.path.join(project_path, imp)) and not os.path.exists(
            os.path.join(project_path, f"{imp}.py")
        ):
