...
class ExpenseLineItemSerializer(TenantSerializerMixin, serializers.ModelSerializer):
    ...
    def validate(self, data):
        expense_type = data.get('expense_type') or (self.instance.expense_type if self.instance else None)
        ...
        if expense_type == 'vendor' and not vendor:
            raise serializers.ValidationError({"vendor": "A vendor is required for vendor-type expenses."})
        if expense_type == 'item' and not name:
            raise serializers.ValidationError({"name": "An item name is required for item-type expenses."})
        return data
...
class ChecklistTaskSerializer(TenantSerializerMixin, serializers.ModelSerializer):
    ...
    class Meta:
        model = ChecklistTask
        fields = [
            'task_id', 'event', 'checklist_type', 'parent_task', 'name', 'description',
            'due_date', 'is_done', 'position', 'subtasks', 'created_at', 'updated_at'
        ]
        read_only_fields = ['created_at', 'updated_at']

    def get_subtasks(self, obj):
        return ChecklistTaskSerializer(obj.subtasks.all(), many=True, context=self.context).data
