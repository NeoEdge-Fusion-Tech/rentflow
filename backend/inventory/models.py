...
class Product(models.Model):
    ...
    def get_availability(self, start_date, end_date, exclude_booking_id=None):
        ...
        overlapping_items_sum = (
            BookingItem.objects.filter(q).aggregate(
    ...
    def get_available_units(self, start_date, end_date, exclude_booking_id=None):
        ...
        reserved_counts = (
            BookingItemUnit.objects.filter(q)
            .values("product_unit_id")
            .annotate(total_reserved=Sum("quantity"))
        )
        reserved_dict = {
            rc["product_unit_id"]: rc["total_reserved"] for rc in reserved_counts
        }
...
class ProductUnit(models.Model):
    ...
    unit = models.CharField(
        max_length=20, blank=True, null=True, choices=unit_choices, default="per_day"
    )
    ...
    def sync_quantities(self):
        ...
        if self.unit_type == "bulk":
            ...
            self.quantity_rented = min(
                currently_rented, qty_good
            )  # can't exceed good qty
...
class BookingItem(models.Model):
    ...
    created_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="created_booking_items",
    )
    updated_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="updated_booking_items",
    )
...
class BookingItemUnit(models.Model):
    ...
    booking_item = models.ForeignKey(
        BookingItem, on_delete=models.CASCADE, related_name="units"
    )
    ...
    organization = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="booking_item_units",
    )
    quantity = models.PositiveIntegerField(default=1)
    ...
    created_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="created_booking_item_units",
    )
    updated_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="updated_booking_item_units",
    )
...
def update_booking_and_item_status(sender, instance, **kwargs):
    ...
    if instance.booking_item:
        ...
        item.total_returned = (agg["total_returned_good"] or 0) + (
            agg["total_returned_damaged"] or 0
        )
