...
class ProductViewSet(TenantIsolationMixin, viewsets.ModelViewSet):
    ...
    def availability(self, request, pk=None):
        ...
        available_qty = product.get_availability(
            start_date, end_date, exclude_booking_id=exclude_booking_id
        )
        ...
        if exclude_booking_id:
            ...
            bus = BookingItemUnit.objects.filter(
                booking_item__booking_id=exclude_booking_id
            )
            for bu in bus:
            ...
            "available_units": [
                {
                    ...
                    "quantity_picked_up": booking_units_stats.get(
                        u.product_unit_id, {}
                    ).get("quantity_picked_up", 0),
                    "quantity_returned_good": booking_units_stats.get(
                        u.product_unit_id, {}
                    ).get("quantity_returned_good", 0),
                    "quantity_returned_damaged": booking_units_stats.get(
                        u.product_unit_id, {}
                    ).get("quantity_returned_damaged", 0),
                }
...
class BookingFilter(django_filters.FilterSet):
    pickup_after = django_filters.DateTimeFilter(
        field_name="pickup_date", lookup_expr="gte"
    )
    pickup_before = django_filters.DateTimeFilter(
        field_name="pickup_date", lookup_expr="lte"
    )
    return_after = django_filters.DateTimeFilter(
        field_name="return_date", lookup_expr="gte"
    )
            ...
            if plan:
                if not plan.has_booking:
                    raise PermissionDenied(
                        "Your current subscription plan does not include the Inventory Booking module. Please upgrade to continue."
                    )
                ...
                if quota != -1:
                    ...
                    if current_month_bookings >= quota:
                        raise PermissionDenied(
                            f"You have reached your limit of {quota} bookings per month. Please upgrade your plan to continue."
                        )
...
class TenantStatsAPIView(APIView):
    ...
    def get(self, request):
        ...
        revenue_agg = Booking.objects.filter(
            organization_id=organization.id, created_at__gte=start_of_month
        ...
        return Response(
            {
                ...
                "currency_symbol": (
                    organization.currency.symbol if organization.currency else "$"
                ),
...
class ScanItemAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "Unit not found on this account"},
        ...
        if action == "verify":
            ...
            if unit.unit_type == "bulk":
                ...
                if not booking_unit:
                    return Response(
                        {"error": "No pending bulk booking items found for this unit"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                ...
                if qty > remaining:
                    return Response(
                        {
                            "error": f"Quantity exceeds remaining booking amount ({remaining})"
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                ...
                item.total_picked_up = (
                    item.units.aggregate(total=Sum("quantity_picked_up"))["total"] or 0
                )
                item.save()
                ...
                if not booking_unit:
                    return Response(
                        {"error": "No pending booking found for this unit"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
            ...
            if unit.unit_type == "bulk":
                booking_unit = (
                    BookingItemUnit.objects.filter(
                        ...
                        quantity_picked_up__gt=models.F("quantity_returned_good")
                        + models.F("quantity_returned_damaged"),
                    )
                    .select_related(
                        "booking_item__booking", "booking_item__booking__client"
                    )
                ...
                if not booking_unit:
                    return Response(
                        {"error": "No items currently picked up for this bulk unit"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )

                if qty_good is None and qty_damaged is None:
                    total_out = booking_unit.quantity_picked_up - (
                        booking_unit.quantity_returned_good
                        + booking_unit.quantity_returned_damaged
                    )
                    return Response(
                        {
                            "requires_quantity": True,
                ...
                try:
                    ...
                    if q_good < 0 or q_damaged < 0 or (q_good + q_damaged) <= 0:
                        raise ValueError()
                except:
                    return Response(
                ...
                if total_returning > total_out:
                    return Response(
                        {
                            "error": f"Quantity exceeds currently out amount ({total_out})"
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                ...
                booking_unit.quantity_returned_in_good_condition = (
                    booking_unit.quantity_returned_good
                )
                booking_unit.quantity_returned_in_damaged_condition = (
                    booking_unit.quantity_returned_damaged
                )
                ...
                if (
                    booking_unit.quantity_returned_good
                    + booking_unit.quantity_returned_damaged
                    == booking_unit.quantity_picked_up
                ):
                ...
                if q_damaged > 0:
                    unit.quantity_damaged = min(
                        unit.quantity, (unit.quantity_damaged or 0) + q_damaged
                    )
                ...
                item.total_returned = (
                    item.units.aggregate(
                        total=Sum("quantity_returned_good")
                        + Sum("quantity_returned_damaged")
                    )["total"]
                    or 0
                )
                item.save()
                ...
                booking_unit = (
                    BookingItemUnit.objects.filter(
                        product_unit=unit, status="picked_up"
                    )
                    .select_related(
                        "booking_item__booking", "booking_item__booking__client"
                    )
                ...
                if not booking_unit:
                    return Response(
                        {
                            "error": "This item is not currently marked as picked up in any booking"
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
