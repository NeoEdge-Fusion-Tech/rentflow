...
class ProductUnitSerializer(TenantSerializerMixin, serializers.ModelSerializer):
    ...
    def validate_quantity_damaged(self, value):
        ...
        quantity = (
            self.initial_data.get("quantity") if hasattr(self, "initial_data") else None
        )
    ...
    def validate(self, data):
        ...
        if serial:
            ...
            if qs.exists():
                raise serializers.ValidationError(
                    {
                        "serial_number": "product unit with this serial number already exists."
                    }
                )
        return data
...
class ProductSerializer(TenantSerializerMixin, serializers.ModelSerializer):
    ...
    def validate(self, data):
        if not self.instance and not data.get("units"):
            raise serializers.ValidationError(
                {
                    "units": "At least one product unit is required when creating a product."
                }
            )
        return data
...
class BookingSerializer(TenantSerializerMixin, serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if not self.instance and not data.get("items"):
            raise serializers.ValidationError(
                {
                    "items": "At least one booking item is required when creating a booking."
                }
            )
    ...
    def create(self, validated_data):
        ...
        for item_data in items_data:
            ...
            item_data["total_price"] = (
                item_data["unit_price"] * item_data["quantity_booked"]
            )
    ...
    def update(self, instance, validated_data):
        ...
        if items_data is not None:
            ...
            existing_items = {
                item.booking_item_id: item for item in instance.items.all()
            }
            ...
            for item_data in items_data:
                ...
                if units_data is not None:
                    existing_units = {
                        u.booking_item_unit_id: u for u in booking_item.units.all()
                    }
                    ...
                    for unit_entry in units_data:
                        ...
                        unit_fields = {
                            "product_unit": unit_entry["product_unit"],
                            "quantity": unit_entry.get("quantity", 1),
                            "quantity_picked_up": unit_entry.get(
                                "quantity_picked_up", 0
                            ),
                            "quantity_returned_good": unit_entry.get(
                                "quantity_returned_good", 0
                            ),
                            "quantity_returned_damaged": unit_entry.get(
                        ...
                        if unit_id and unit_id in existing_units:
                            ...
                            BookingItemUnit.objects.create(
                                booking_item=booking_item, **unit_fields
                            )
