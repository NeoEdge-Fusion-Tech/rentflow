...
class EmailService:
    ...
    def send(to_email, subject, body_text, body_html=None):
        ...
        """
        environment = getattr(
            settings, "ENVIRONMENT", config("ENVIRONMENT", default="local")
        ).lower()
        if (
            environment == "local"
            and getattr(settings, "EMAIL_VENDOR", "resend").lower() != "resend"
        ):
            logger.info(
                f"[LOCAL EMAIL SIMULATOR] To: {to_email} | Subject: {subject} | Body: {body_text}"
            )
            return True
        ...
        if vendor == "resend":
            ...
            return EmailService.send_via_sendgrid(
    ...
    def send_via_resend(to_email, subject, body_text, body_html=None):
        try:
            ...
            resend.api_key = getattr(
                settings, "RESEND_API_KEY", config("RESEND_API_KEY", default="")
            )
            ...
            r = resend.Emails.send(
                {
                    "from": config(
                        "DEFAULT_FROM_EMAIL", default="onboarding@resend.dev"
                    ),
                    "to": to_email,
                    "subject": subject,
    ...
    def send_via_ses(to_email, subject, body_text, body_html=None):
        try:
            client_ses = boto3.client(
                "ses", region_name=config("AWS_REGION", default="us-east-1")
            )
            response = client_ses.send_email(
                ...
                Source=config("DEFAULT_FROM_EMAIL", default="noreply@neoops.com"),
            )
            logger.info(
                f"SES Success: Sent email to {to_email} - MessageId: {response['MessageId']}"
            )
            return True
    ...
    def send_via_sendgrid(to_email, subject, body_text, body_html=None):
        try:
            ...
            message = Mail(
                from_email=config("DEFAULT_FROM_EMAIL", default="noreply@neoops.com"),
                to_emails=to_email,
                subject=subject,
            ...
            logger.info(
                f"SendGrid Success: Sent to {to_email} - Status: {response.status_code}"
            )
            return True
        except Exception as sg_e:
            logger.error(
                f"SendGrid Failure: Could not send email to {to_email}. Error: {str(sg_e)}"
            )
            return False
...
def send_credit_alert_email(user, current_credits, topup_url):
    ...
    """
    subject = "NeoOps - Low Credit Alert"
    ...
    body_html = render_to_string(
        "emails/credit_alert.html",
        {"user": user, "current_credits": current_credits, "topup_url": topup_url},
    )
    dispatch_email(
