...
export function OrganizationSelector() {
  ...
  const selectedOrg = organizations.find(
    (org) => org.id.toString() === selectedId,
  );
      ...
      >
        <div
          className={`p-1 rounded-lg ${
            selectedId
              ? "bg-brand-accent/10"
              : "bg-[var(--bg-app)] group-hover:bg-brand-primary/10 transition-colors"
          }`}
        >
          <Building2
            className={`w-3.5 h-3.5 ${
              selectedId
                ? "text-brand-accent"
                : "text-[var(--text-muted)] group-hover:text-brand-primary"
            }`}
          />
        </div>
        ...
        <ChevronDown
          className={`w-3.5 h-3.5 transition-transform duration-300 ${
            ...
            selectedId ? "text-brand-accent/60" : "text-[var(--text-muted)]"
          }`}
        />
      </button>
                ...
                <div className="flex items-center gap-3">
                  <div
                    className={`p-1.5 rounded-lg ${
                      !selectedId
                        ? "bg-brand-accent/20"
                        : "bg-[var(--bg-app)] group-hover:bg-[var(--bg-surface)]"
                    }`}
                  >
                    <Globe
                      className={`w-3.5 h-3.5 ${
                      ...
                      <div
                        className={`shrink-0 w-8 h-8 rounded-lg flex items-center justify-center font-black ${
                          selectedId === org.id.toString()
                            ...
                            : "bg-[var(--bg-app)] text-[var(--text-muted)] group-hover:bg-brand-primary/10 group-hover:text-brand-primary transition-colors"
                        }`}
                      >
                        {org.name[0]}
                      </div>
