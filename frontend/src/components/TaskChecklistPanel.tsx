...
export function TaskChecklistPanel({
  ...
  useEffect(() => {
    (async () => {
      ...
      await Promise.all([
        fetchTasks(),
        EventService.getAll()
          .then((res) => setAllEvents(res.data.results || res.data))
          .catch(console.error),
      ]);
      setIsLoading(false);
    })();
  ...
  const handleToggleDone = async (task: any) => {
    try {
      await ChecklistTaskService.update(task.task_id, {
  ...
  const reorderSiblings = async (
    ...
    const movedIds = new Set([
      siblings[fromIndex].task_id,
      siblings[toIndex].task_id,
    ]);
    ...
    try {
      await Promise.all(
        reordered.map((t, idx) =>
          t.position === idx
            ? Promise.resolve()
            : ChecklistTaskService.update(t.task_id, { position: idx }),
        ),
      );
      fetchTasks();
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-end gap-2">
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 text-xs font-bold bg-[var(--bg-surface)] border border-[var(--border-soft)] text-[var(--text-main)] rounded-xl hover:bg-[var(--bg-app)] transition-colors"
        >
          <Copy className="w-4 h-4" />{" "}
          {isChecklistEmpty
            ? "Duplicate from Project"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {CHECKLIST_SECTIONS.map((section) => (
          <div
            key={section.key}
            className="bg-[var(--bg-surface)] rounded-2xl border border-[var(--border-soft)] p-5 space-y-3"
          >
            <h3 className="font-bold text-[var(--text-main)] text-sm uppercase tracking-wider">
              {section.label}
            </h3>

            {tasksByType[section.key].length === 0 &&
            ...
            <DndContext
              ...
              onDragEnd={(e) => handleTaskDragEnd(section.key, e)}
            >
              <SortableContext
                items={tasksByType[section.key].map((t: any) => t.task_id)}
              ...
              >
                {tasksByType[section.key].map((task: any) => (
                  <SortableTaskWrapper
                    key={task.task_id}
                    id={task.task_id}
                    disabled={editingTaskId === task.task_id}
                  >
                    {(dragHandleProps) => (
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                              <button
                                ...
                                className="flex-1 py-1.5 text-xs font-bold text-[var(--text-muted)] bg-[var(--bg-surface)] border border-[var(--border-soft)] rounded-md"
                              >
                                Cancel
                              </button>
                              <button
                                onClick={() => handleSaveTaskEdit(task.task_id)}
                                disabled={isSavingTaskEdit}
                                className="flex-1 py-1.5 text-xs font-bold text-white bg-brand-primary rounded-md disabled:opacity-50"
                              >
                        ...
                        {(expandedTasks[task.task_id] ||
                          ...
                          <div className="ml-7 mt-2 space-y-1.5 border-l border-[var(--border-subtle)] pl-3">
                                  ...
                                  {editingSubtaskId === sub.task_id ? (
                                    <>
                                      <input
                                        ...
                                        onChange={(e) =>
                                          setEditSubtaskName(e.target.value)
                                        }
                                        onKeyDown={(e) => {
                                          if (e.key === "Enter")
                                            handleSaveSubtaskEdit(sub.task_id);
                                          if (e.key === "Escape")
                                            setEditingSubtaskId(null);
                                        }}
                                      ...
                                      <button
                                        ...
                                        className="text-xs font-bold text-brand-primary disabled:opacity-50"
                                      >
                                        Save
                                      </button>
                                    </>
                                    ...
                                    <>
                                      <div className="flex flex-col shrink-0 -my-0.5">
                                        <button
                                          onClick={() =>
                                            handleMoveSubtask(task, sub, "up")
                                          }
                                          disabled={
                                            subIndex === 0 ||
                                            reorderingIds.has(sub.task_id)
                                          }
                                        ...
                                        <button
                                          onClick={() =>
                                            handleMoveSubtask(task, sub, "down")
                                          }
                                          disabled={
                                            subIndex ===
                                              (task.subtasks?.length || 0) -
                                      ...
                                      <p
                                        className={cn(
                                          "text-xs font-medium text-[var(--text-main)] flex-1",
                                          sub.is_done &&
                                            "line-through text-[var(--text-muted)]",
                                        )}
                                      >
                                      ...
                                      <button
                                        ...
                                        className="p-0.5 text-[var(--text-muted)] hover:text-brand-primary"
                                      >
                                        <Edit2 className="w-3 h-3" />
                                      </button>
                                      <button
                                        onClick={() =>
                                          handleDeleteTask(sub.task_id)
                                        }
                            ...
                            {addingSubtaskFor === task.task_id ? (
                              ...
                              <button
                                onClick={() => {
                                  setAddingSubtaskFor(task.task_id);
                                  setNewSubtaskName("");
                                }}
                                className="text-[11px] font-bold text-brand-primary hover:underline pt-1"
                              >
                                + Add subtask
                              </button>
                            )}
            ...
            {addingTaskFor === section.key ? (
              ...
              <button
                onClick={() => openAddTask(section.key)}
                className="w-full flex items-center justify-center gap-1.5 py-2 text-xs font-bold text-brand-primary border border-dashed border-[var(--border-soft)] rounded-xl hover:bg-[var(--bg-app)] transition-colors"
              >
          ...
          <div className="bg-[var(--bg-surface)] rounded-3xl w-full max-w-md border border-[var(--border-soft)] shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-[var(--border-soft)] bg-[var(--bg-app)]/50">
              <h2 className="text-lg font-bold text-[var(--text-main)]">
                {isChecklistEmpty
                  ? "Duplicate Checklist From Project"
                  : "Duplicate Checklist"}
              </h2>
              <button
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-[var(--text-muted)]">
                  {isChecklistEmpty ? "Source Project" : "Target Project"}
                </label>
                <select
                ...
                >
                  ...
                  {allEvents
                    .filter((e: any) => String(e.event_id) !== String(eventId))
                    .map((e: any) => (
              ...
              <div className="pt-2 flex gap-3">
                ...
                <button
                  ...
                  disabled={!duplicateTarget || isDuplicating}
                  className="flex-1 px-6 py-2.5 bg-brand-primary text-white font-bold rounded-xl hover:bg-brand-secondary transition-colors disabled:opacity-50"
                >
