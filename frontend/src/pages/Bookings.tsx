...
export function Bookings() {
  ...
  const [isGeneratingPaymentLink, setIsGeneratingPaymentLink] = useState<
    any | null
  >(null);
  ...
  const fetchBillingDocs = async (bookingId: any) => {
    try {
      ...
      const recRes = await ReceiptService.getAll({
        payment__booking: bookingId,
      });
  ...
  const [activeManagerTab, setActiveManagerTab] = useState<"items" | "billing">(
    "items",
  );
  ...
  const [availableUnitsMap, setAvailableUnitsMap] = useState<
    Record<string, any[]>
  >({});
  ...
  const fetchAvailability = async (index: number, productId: string) => {
    ...
    try {
      const res = await ProductService.getAvailability(
        productId,
  ...
  const validateForm = () => {
    ...
    if (bookingItems.some((i) => !i.product_id)) {
      showNotification(
        "One or more items have no product selected.",
        "warning",
      );
      return false;
    }
    ...
    // Overbooked check
    const overbookedItem = bookingItems.find(
      (i) => i.quantity_booked > i.available_qty,
    );
    if (overbookedItem) {
      const prod = products.find(
        (p) => p.product_id === parseInt(overbookedItem.product_id),
      );
      showNotification(
        `Overbooked: Only ${overbookedItem.available_qty} of "${
          prod?.name || "product"
        }" available.`,
        "error",
      );
  ...
  const handleUpdateAmountPaid = async (
    ...
    try {
      setIsLoading(true);
      const res = await BookingService.patch(bookingId, {
  ...
  const handleSaveBookingEdits = async () => {
    ...
    if (
      new Date(editFormData.pickup_date) >= new Date(editFormData.return_date)
    ) {
      showNotification("Return date must be after pickup date.", "warning");
      return;
    }
    ...
    try {
      ...
      // Overbooking check
      const overbookedItem = editFormData.items.find(
        (i: any) => i.quantity_booked > (i.available_qty ?? Infinity),
      );
      if (overbookedItem) {
        showNotification(
          `Overbooked: Only ${overbookedItem.available_qty} of "${overbookedItem.product_name}" available.`,
          "error",
        );
        return;
      }
  ...
  const calculateEditTotal = () => {
    ...
    const itemsTotal = editFormData.items.reduce(
      (acc: number, item: any) =>
        acc + parseFloat(item.unit_price) * item.quantity_booked,
      0,
    );
    ...
    const discountVal =
      discPercentage > 0 ? itemsTotal * (discPercentage / 100) : discAmount;

    return Math.max(0, itemsTotal - discountVal);
  };

  const fetchAvailableUnitsForManage = async (
    productId: string,
    bookingId: number,
    start: string,
    end: string,
  ...
  const totalBeforeDiscount = bookingItems.reduce(
    (acc, item) => acc + item.unit_price * item.quantity_booked,
    0,
  );
  const discountVal =
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[var(--text-muted)]">
            Track rentals, schedules, and fulfillment status.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mt-4 mb-2">
        {[
          {
            label: "Total",
            value: stats?.booking_stats?.total,
            color: "slate",
          },
          {
            label: "Pending",
          ...
          {
            label: "Confirmed",
            value: stats?.booking_stats?.confirmed,
            color: "blue",
          },
          {
            label: "Picked Up",
            value: stats?.booking_stats?.picked_up,
            color: "indigo",
          },
          {
            label: "Returned",
            value: stats?.booking_stats?.returned,
            color: "emerald",
          },
          {
            label: "Completed",
            value: stats?.booking_stats?.completed,
            color: "brand",
          },
        ].map((s, idx) => (
          <div
        ...
        <div className="flex items-center justify-between mt-4 border-t border-[var(--border-soft)] pt-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() =>
                setCurrentPage((p) =>
                  Math.min(Math.ceil(bookings.length / itemsPerPage), p + 1),
                )
              }
              className="p-2 bg-[var(--bg-surface)] border border-[var(--border-soft)] rounded-lg text-[var(--text-main)] hover:bg-[var(--bg-app)] disabled:opacity-50"
              disabled={
            ...
            <div className="flex items-center justify-between px-8 py-6 border-b border-[var(--border-subtle)] bg-[var(--bg-app)]/50">
              <div>
                ...
                <p className="text-xs text-[var(--text-muted)] font-medium mt-0.5">
                  Fill in the details to create a new rental reservation.
                </p>
              </div>
              <button
                  ...
                  <section className="space-y-6">
                    ...
                    <div className="space-y-4 pl-11">
                      ...
                      {bookingItems.map((item, i) => (
                        <div
                          key={i}
                          className="group relative bg-[var(--bg-surface)] border border-[var(--border-soft)] rounded-[1.25rem] p-5 hover:border-brand-primary transition-all hover:shadow-xl hover:shadow-brand-primary/5"
                        >
                          <div className="grid grid-cols-12 gap-5 items-start">
                            ...
                            <div className="col-span-6 md:col-span-2 space-y-2">
                              <label className="block text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest text-center">
                                Qty
                              </label>
                              <input
                                type="number"
                            ...
                            <div className="col-span-6 md:col-span-3 space-y-2 text-right">
                              <label className="block text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest pr-1">
                                Unit Price
                              </label>
                              <div className="relative">
                                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-muted)] font-bold text-sm">
                                  {currencySymbol}
                                </span>
                                <input
                                  type="number"
                            ...
                            <div className="col-span-12 md:col-span-2 pt-6 flex justify-end">
                              <button
                                onClick={() =>
                                  setBookingItems(
                                    bookingItems.filter((_, idx) => idx !== i),
                                  )
                                }
                    ...
                    <div className="space-y-4 pl-11">
                      <div className="grid grid-cols-2 gap-2 p-1 bg-[var(--bg-app)] rounded-[1.25rem] w-full md:w-64 border border-[var(--border-soft)]">
                        <button
                          ...
                          className={cn(
                            "py-2.5 text-[10px] font-black rounded-xl transition-all uppercase tracking-widest",
                ...
                <div className="lg:col-span-4">
                  <div className="sticky top-0 space-y-6">
                    <div className="bg-[var(--bg-surface)] rounded-[2.5rem] p-8 text-[var(--text-main)] shadow-2xl border border-[var(--border-soft)] transition-colors">
                      <h3 className="text-lg font-black uppercase tracking-[0.2em] mb-10 text-[var(--text-muted)] opacity-60">
                        Order Summary
                      </h3>
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="pt-10 space-y-4">
                          <div className="flex justify-between items-baseline gap-2 flex-wrap min-h-[3rem]">
                            <span className="text-sm font-black uppercase tracking-[0.1em] text-[var(--text-muted)]">
                              Total Amount
                            </span>
                    ...
                    <div className="p-6 bg-[var(--bg-app)] rounded-3xl border border-[var(--border-soft)] italic">
                      <p className="text-[10px] text-[var(--text-muted)] font-bold leading-relaxed tracking-wider uppercase">
                        By proceeding, you verify all items are in good
                        condition and the rental dates are locked. All
                        availability checks are final for the selected window.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="lg:col-span-5 p-8 space-y-8 border-r border-[var(--border-soft)] overflow-y-auto custom-scrollbar max-h-[70vh]">
                      ...
                      <div className="flex flex-wrap gap-2">
                              ...
                              : selectedBooking.status === "picked_up"
                                ...
                                : selectedBooking.status === "returned"
                                  ...
                                  : selectedBooking.status === "completed"
                                    ? "bg-emerald-500/20 text-emerald-600 border-emerald-500/30 font-black shadow-sm"
                                    : "bg-[var(--bg-app)] text-[var(--text-muted)] border-[var(--border-soft)]",
                          )}
                        >
                        ...
                        <span
                          className={cn(
                            ...
                            parseFloat(selectedBooking.amount_paid) >=
                              ...
                              ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20"
                              : parseFloat(selectedBooking.amount_paid) > 0
                        ...
                        >
                          ...
                          parseFloat(selectedBooking.total_amount)
                            ? "Fully Paid"
                            : parseFloat(selectedBooking.amount_paid) > 0
                              ? `Partially Paid (${currencySymbol}${formatCurrency(
                ...
                <section className="space-y-6">
                  ...
                  <div className="space-y-4 pl-11">
                    {isManagingBooking && (
                      <div className="grid grid-cols-2 gap-2 p-1 bg-[var(--bg-app)] rounded-xl w-full md:w-48 border border-[var(--border-soft)] mb-2">
                        <button
                          ...
                          className={cn(
                            "py-1.5 text-[9px] font-black rounded-lg transition-all uppercase tracking-widest",
                            editFormData.delivery_mode === "pickup"
                              ? "bg-brand-primary text-brand-accent shadow-sm"
                              : "text-[var(--text-muted)] hover:text-[var(--text-main)]",
                          )}
                        >
                        ...
                        <button
                          ...
                          className={cn(
                            "py-1.5 text-[9px] font-black rounded-lg transition-all uppercase tracking-widest",
                            editFormData.delivery_mode === "delivery"
                              ? "bg-brand-primary text-brand-accent shadow-sm"
                              : "text-[var(--text-muted)] hover:text-[var(--text-main)]",
                          )}
                        >
                    ...
                    <AnimatePresence>
                      {(isManagingBooking
                        ? editFormData?.delivery_mode
                        : selectedBooking.delivery_mode) === "delivery" && (
                        <motion.div
                          initial={{ height: 0, opacity: 0 }}
                          animate={{ height: "auto", opacity: 1 }}
                          exit={{ height: 0, opacity: 0 }}
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-1">
                            <div>
                              ...
                              {isManagingBooking ? (
                                ...
                                <p className="text-sm font-medium text-[var(--text-main)]">
                                  {selectedBooking.contact_name || "N/A"}
                                </p>
                              )}
                            </div>
                            <div>
                              ...
                              {isManagingBooking ? (
                                ...
                                <p className="text-sm font-medium text-[var(--text-main)]">
                                  {selectedBooking.contact_phone || "N/A"}
                                </p>
                              )}
                            </div>
                          </div>
                    ...
                    {!isManagingBooking &&
                      selectedBooking.delivery_mode === "pickup" && (
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          activeManagerTab === "items"
                            ? "bg-[var(--bg-surface)] text-brand-primary shadow-sm"
                            : "text-[var(--text-muted)] hover:text-[var(--text-main)]",
                        )}
                      >
                        Line Items
                        ...
                        {!selectedBooking.has_invoice && !isManagingBooking && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              navigate(
                                `/invoices/new?booking_id=${selectedBooking.booking_id}`,
                              );
                            }}
                  ...
                  {!isManagingBooking && (
                    <div className="flex flex-wrap gap-2 pb-4 border-b border-[var(--border-subtle)]">
                      {selectedBooking.invoices_summary?.map((inv: any) => (
                        <div
                          key={inv.invoice_id}
                          className="flex items-center bg-blue-500/5 rounded-xl border border-blue-500/10 shadow-sm overflow-hidden group"
                        >
                          <button
                            onClick={() =>
                              handleDownloadDocument(
                          ...
                          >
                            <FileText className="w-3.5 h-3.5" /> INV:{" "}
                            {inv.invoice_number}
                          </button>
                          <button
                            onClick={() =>
                      ...
                      {selectedBooking.receipts_summary?.map((r: any) => (
                        <div
                          key={r.receipt_id}
                          className="flex items-center bg-emerald-500/5 rounded-xl border border-emerald-500/10 shadow-sm overflow-hidden group"
                        >
                          <button
                            onClick={() =>
                              handleDownloadDocument(
                          ...
                          >
                            <Receipt className="w-3.5 h-3.5" /> REC:{" "}
                            {r.receipt_number}
                          </button>
                          <button
                            onClick={() => {
                              ...
                              <div className="text-right ml-4">
                                {isManagingBooking ? (
                                  <div className="relative inline-block w-24">
                                    <span className="absolute left-2 top-1/2 -translate-y-1/2 text-[var(--text-muted)] font-bold text-xs">
                                      {currencySymbol}
                                    </span>
                                    <input
                                      type="number"
                                      ...
                                      {availableUnitsMap[
                                        ...
                                        >
                                          <div className="flex flex-col items-center gap-1">
                                            ...
                                            {(unit.quantity_picked_up > 0 ||
                                              unit.quantity_returned_good > 0 ||
                                              unit.quantity_returned_damaged >
                                                0) && (
                                              <div className="flex gap-1.5 text-[8px] opacity-80 font-black tracking-tighter uppercase">
                                                <span className="text-blue-200">
                                                  P:{unit.quantity_picked_up}
                                                </span>
                                                <span className="text-emerald-200">
                                                  G:
                                                  {unit.quantity_returned_good}
                                                </span>
                                      ...
                                      {(!availableUnitsMap[
                                        ...
                                        <div className="w-full p-3 bg-amber-500/10 rounded-xl border border-amber-500/20">
                                          <p className="text-[10px] text-amber-500 font-bold italic">
                                            No physical units available for
                                            these dates.
                                          </p>
                                        </div>
                                      )}
                                  ...
                                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    {item.units?.map(
                                      (unit: any, ui: number) => (
                                        <div
                                          key={ui}
                                          className="p-3 bg-[var(--bg-app)] border border-[var(--border-soft)] rounded-xl shadow-sm flex flex-col gap-2"
                                        >
                                          <div className="flex items-center justify-between">
                                                  ...
                                                  <button
                                                    onClick={() =>
                                                      updateUnitTracking(
                                                        ...
                                                        ui,
                                                        "quantity_picked_up",
                                                        unit.quantity_picked_up
                                                          ? 0
                                                          : 1,
                                                      )
                                                    }
                                                    className={cn(
                                                      "mt-1 text-[10px] font-bold px-1.5 py-0.5 rounded uppercase leading-none",
                                                      unit.quantity_picked_up
                                                        ? "bg-emerald-500 text-brand-accent"
                                    ...
                                    {(!item.units ||
                                      item.units.length === 0) && (
                                      <span className="text-[10px] text-[var(--text-muted)] italic font-medium">
                                        No units assigned to this booking yet.
                                      </span>
                                    )}
                                  </div>
                                )}
                      ...
                      <div className="pt-6 border-t border-[var(--border-subtle)] p-2 space-y-4">
                        ...
                        <div className="flex flex-wrap gap-2 p-3 bg-[var(--bg-app)] rounded-2xl border border-[var(--border-soft)]">
                          <div className="flex-1 min-w-[140px] space-y-1">
                            <label className="block text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest">
                              Update Amount Paid ({currencySymbol})
                            </label>
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                      ...
                      <div className="bg-[var(--bg-surface)] p-6 rounded-[2rem] border border-[var(--border-soft)] shadow-sm">
                        ...
                        {bookingInvoices.map((inv: any) => (
                          <div
                            key={inv.invoice_id}
                            className="mt-4 p-4 bg-[var(--bg-app)] rounded-2xl border border-[var(--border-soft)] group relative overflow-hidden"
                          >
                            ...
                            <div className="mt-4 pt-4 border-t border-[var(--border-soft)] flex justify-between items-center relative z-10">
                              <p className="text-[10px] text-[var(--text-muted)] font-black uppercase tracking-widest text-left">
                                Status: {inv.status}
                              </p>
                      ...
                      <div className="bg-[var(--bg-surface)] p-6 rounded-[2rem] border border-[var(--border-soft)] shadow-sm">
                          ...
                          <div className="mb-6 p-4 bg-blue-500/10 border border-blue-500/20 rounded-2xl animate-in slide-in-from-top-2">
                            ...
                            <div className="flex items-center gap-2">
                              <input
                                readOnly
                                value={paymentLink}
                                className="flex-1 bg-[var(--bg-surface)] border border-[var(--border-soft)] rounded-xl px-3 py-2 text-xs font-mono text-[var(--text-main)] outline-none"
                              />
                              <button
                                onClick={() => {
                        ...
                        <div className="space-y-4">
                          {(selectedBooking.payments || []).map(
                            (payment: any) => {
                              const receipt = bookingReceipts.find(
                                (r) => r.payment === payment.payment_id,
                              );
                              return (
                                <div
                                  key={payment.payment_id}
                                  className="p-4 bg-[var(--bg-app)] rounded-2xl border border-[var(--border-soft)] flex items-center justify-between group"
                                >
                                  <div className="space-y-1">
                                    <div className="flex items-center gap-2">
                                      <p className="text-xs font-black text-[var(--text-main)]">
                                    ...
                                    {receipt && (
                                      <div className="mt-2 flex items-center gap-2">
                                        <p className="text-[11px] font-bold text-emerald-500 tracking-tight">
                                          Receipt #{receipt.receipt_number}
                                        </p>
                                        {receipt.notes && (
                                          <span className="text-[10px] text-[var(--text-muted)] italic leading-none border-l border-emerald-500/20 pl-2 ml-1">
                                            {receipt.notes}
                                          </span>
                                        )}
                                      </div>
                                        ...
                                        <button
                                          onClick={() =>
                                            handleDownloadDocument(
                                              "receipt",
                                              receipt.receipt_id,
                                              `receipt_${receipt.receipt_number}`,
                                            )
                                          }
                                          className="p-2 text-[var(--text-muted)] hover:text-brand-primary hover:bg-[var(--bg-surface)] rounded-lg transition-all"
                                          title="Download PDF"
            ...
            <div className="grid grid-cols-1 gap-4">
              {paymentLink ? (
                <div className="p-4 bg-[var(--bg-app)] rounded-2xl border border-[var(--border-soft)] animate-in slide-in-from-bottom-2">
                  <p className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest mb-2 text-left">
                ...
                <button
                  onClick={() => {
                    ...
                    setPaymentLinkAmount(
                      lastCreatedBooking.total_amount -
                        (lastCreatedBooking.amount_paid || 0),
                    );
                  }}
                  disabled={isLoading}
              ...
              <button
                onClick={() =>
                  handleUpdateStatus(lastCreatedBooking.booking_id, "confirmed")
                }
              ...
              <button
                onClick={() =>
                  navigate(
                    `/invoices/new?booking_id=${lastCreatedBooking.booking_id}`,
                  )
                }
          ...
          <div className="bg-[var(--bg-surface)] rounded-[2rem] w-full max-w-lg shadow-2xl border border-[var(--border-soft)] animate-in zoom-in-95 duration-200">
            <div className="px-8 py-6 border-b border-[var(--border-soft)] flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-black text-[var(--text-main)] uppercase tracking-widest">
                  Edit {editingDoc.type}
                </h3>
              </div>
              <button
            ...
            <div className="p-8 space-y-6">
              {editingDoc.type === "invoice" && (
                <div className="space-y-4">
                  <div>
                    <label className="block text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest mb-1.5 pl-1">
                      Due Date
                    </label>
                    <input
                      type="date"
                      value={
                        editingDoc.data.due_date
                          ? editingDoc.data.due_date.split("T")[0]
                          : ""
                      }
                      onChange={(e) =>
                        setEditingDoc({
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setEditingDoc({
                      ...editingDoc,
                      data: { ...editingDoc.data, notes: e.target.value },
                    })
                  }
              ...
              <button
                onClick={() =>
                  handleUpdateDocument(
                    editingDoc.type,
                    editingDoc.type === "invoice"
                      ? editingDoc.data.invoice_id
                      : editingDoc.data.receipt_id,
                    editingDoc.data,
                  )
                }
                disabled={isLoading}
            ...
            <div className="p-8 max-h-[60vh] overflow-y-auto space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pb-8 border-b border-[var(--border-soft)]">
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm font-bold text-[var(--text-main)]">
                    No: {editingReceipt.receipt_number}
                  </p>
                  <p className="text-xs text-[var(--text-muted)]">
                    {new Date(editingReceipt.issue_date).toLocaleDateString()}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 pt-8">
                ...
                <p className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest italic">
                  Official Receipt • Branded by Neo Event
                </p>
              </div>
            </div>
          ...
          <div className="bg-[var(--bg-surface)] rounded-[2rem] w-full max-w-sm shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-[var(--border-soft)]">
            <div className="p-6 border-b border-[var(--border-soft)] flex justify-between items-center bg-[var(--bg-app)]">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-sm font-black text-[var(--text-main)] uppercase tracking-widest">
                  Payment Link
                </h3>
              </div>
              <button
                onClick={() => setIsGeneratingPaymentLink(null)}
                className="p-2 hover:bg-[var(--bg-surface)] rounded-full transition-colors text-[var(--text-muted)]"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="p-8 space-y-6">
              <div>
                ...
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm font-bold text-[var(--text-muted)]">
                    {currencySymbol}
                  </span>
                  <input
                    type="number"
                    ...
                    onChange={(e) =>
                      setPaymentLinkAmount(parseFloat(e.target.value))
                    }
