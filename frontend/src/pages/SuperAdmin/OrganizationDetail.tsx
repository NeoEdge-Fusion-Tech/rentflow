...
export function OrganizationDetail() {
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          ...
          showNotification(
            `Organization ${newStatus ? "activated" : "deactivated"}`,
            "success",
          );
          fetchData();
        } catch (error) {
  ...
  const handleSetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await SuperAdminService.setPassword(selectedUser.id, {
  ...
  const handleDeleteOrg = async () => {
    showConfirm({
      ...
      message:
        "Are you SURE you want to delete this organization? This will soft-delete the organization and modify user accounts.",
      type: "danger",
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] p-8 text-center bg-[var(--bg-surface)] rounded-3xl border border-[var(--border-soft)]">
        ...
        <p className="text-[var(--text-muted)] mb-6 max-w-sm">
          The organization you're looking for might have been removed or the ID
          is incorrect.
        </p>
        <button
          onClick={() => navigate("/superadmin/organizations")}
          className="bg-brand-primary text-brand-accent px-6 py-2.5 rounded-xl font-bold shadow-lg shadow-brand-primary/20"
        >
  ...
  const currencySymbol =
    stats?.currency_symbol || localStorage.getItem("currencySymbol") || "$";
          ...
          <div className="flex flex-col gap-1.5 mt-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-[var(--text-main)]">
                {org.subscription?.plan_name || "Free Plan"}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[var(--bg-surface)] p-5 rounded-2xl border border-[var(--border-soft)] shadow-sm md:col-span-2">
          ...
          <div className="grid grid-cols-3 gap-3 mt-2">
            ...
            <div>
              ...
              >
                {formatCurrency(
                  Number((org.revenue || 0) - (org.expenses || 0)),
                  org.currency?.symbol || currencySymbol,
                )}
              </span>
            </div>
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-[var(--bg-app)]/50">
                  <tr>
                    <th className="px-6 py-3 text-[10px] font-bold text-[var(--text-muted)] uppercase">
                      User
                    </th>
                    <th className="px-6 py-3 text-[10px] font-bold text-[var(--text-muted)] uppercase">
                      Role
                    ...
                    <th className="px-6 py-3 text-[10px] font-bold text-[var(--text-muted)] uppercase text-right">
                      Actions
                    </th>
                  </tr>
                </thead>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-brand-primary p-6 rounded-2xl text-brand-accent shadow-lg shadow-brand-primary/20">
            ...
            <p className="text-sm opacity-80 mb-4">
              Manage billing and tier levels for this organization.
            </p>
          ...
          <div className="bg-[var(--bg-surface)] rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-[var(--border-soft)]">
            ...
            <form onSubmit={handleUpdateOrg} className="p-6 space-y-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-[var(--text-muted)] uppercase px-1">
                  Organization Name
                </label>
                <input
                  type="text"
