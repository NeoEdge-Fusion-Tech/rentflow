    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((card, idx) => (
          <div
            key={idx}
            className="bg-[var(--bg-surface)] border border-[var(--border-soft)] rounded-2xl p-6 flex flex-col justify-between hover:border-brand-primary/50 transition-colors group"
          >
            <div className="flex justify-between items-start mb-4">
              <div
                className={`p-3 rounded-xl ${card.bg} ${card.color} group-hover:scale-110 transition-transform`}
              >
        ...
        <div className="bg-[var(--bg-surface)] border border-[var(--border-soft)] rounded-2xl p-6 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto space-y-4">
            {stats.organizations_overview.map((org: any, idx: number) => (
              <div
                key={idx}
                className="flex items-center justify-between p-3 rounded-xl hover:bg-[var(--bg-app)] transition-colors border border-transparent hover:border-[var(--border-soft)]"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[var(--text-muted)] flex items-center gap-1">
                      <CalendarCheck className="w-3 h-3" /> {org.total_bookings}{" "}
                      Bookings
                    </p>
                  </div>
                </div>
      ...
      <div className="bg-[var(--bg-surface)] border border-[var(--border-soft)] rounded-2xl p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-[var(--text-muted)]">
              Latest bookings across all organizations
            </p>
          </div>
        ...
        <div className="space-y-4">
          {stats.recent_activity.map((act: any, idx: number) => (
            <div
              key={idx}
              className="flex items-start gap-4 p-4 rounded-xl border border-[var(--border-soft)] hover:border-brand-primary/30 transition-colors bg-[var(--bg-app)]/30"
            >
              <div className="w-10 h-10 rounded-full bg-blue-500/10 flex items-center justify-center flex-shrink-0">
                <ArrowUpRight className="w-5 h-5 text-blue-500" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-[var(--text-main)] line-clamp-1">
                  {act.title}
                </p>
          ...
          {stats.recent_activity.length === 0 && (
            <div className="text-center py-8 text-[var(--text-muted)]">
              No recent activity across the platform.
            </div>
          )}
        </div>
      </div>
