...
export function Currencies() {
  ...
  const handleCreateCurrency = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showNotification(
        "Failed to create currency. Ensure the code is unique.",
        "error",
      );
    } finally {
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          ...
          fetchCurrencies();
          showNotification(
            `Currency ${newStatus === "active" ? "activated" : "deactivated"}`,
            "success",
          );
        } catch (error) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[var(--text-muted)] text-sm mt-1">
            Manage platform currencies and their active status.
          </p>
        </div>
        <button
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-[var(--bg-app)]/50">
              <tr>
                <th className="px-6 py-4 text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-wider">
                  Currency Details
                </th>
                <th className="px-6 py-4 text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-wider">
                  Symbol
                ...
                <th className="px-6 py-4 text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-wider text-right">
                  Actions
                </th>
              </tr>
            </thead>
          ...
          <div className="bg-[var(--bg-surface)] rounded-3xl w-full max-w-md shadow-2xl overflow-hidden scale-in zoom-in-95 duration-200 border border-[var(--border-soft)]">
            ...
            <form onSubmit={handleCreateCurrency} className="p-6 space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-[var(--text-muted)] uppercase px-1">
                  Currency Name
                </label>
                <input
                  type="text"
