      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[var(--text-muted)]">
            Track invoices, receipts, and transaction status.
          </p>
        </div>
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        ...
                        {payment.invoice_number && (
                          <Link
                            to={`/invoices/${payment.invoice_record}/preview`}
                            className="text-sm text-brand-primary hover:text-brand-primary/80 font-medium flex items-center gap-1 group"
                          >
              ...
              .map((payment) => (
                ...
                >
                  ...
                  <div className="flex gap-4 text-xs text-[var(--text-muted)] pl-11">
                    {payment.booking_ref && (
                      <span>Booking: {payment.booking_ref}</span>
                    )}
                    <span>
                      {new Date(payment.payment_date).toLocaleDateString()}
                    </span>
        ...
        <div className="px-4 py-4 border-t border-[var(--border-soft)] flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() =>
                setCurrentPage((p) =>
                  Math.min(Math.ceil(payments.length / itemsPerPage), p + 1),
                )
              }
              className="p-2 border border-[var(--border-soft)] rounded-lg text-[var(--text-main)] hover:bg-[var(--bg-app)] disabled:opacity-50"
              disabled={
