...
export function BookingValidation() {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    if (qtyDamaged > quantity) {
      showNotification('Damaged quantity cannot exceed total quantity.', 'error');
      return;
    }
    ...
    try {
      ...
      showNotification(`${actionType === 'pickup' ? 'Picked up' : 'Returned'} successfully: ${res.data.product_name}`, 'success');
      ...
      showNotification(err.response?.data?.error || err.message || 'Action failed', 'error');
    } finally {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <p className="text-sm font-bold text-[var(--text-muted)]">Loading booking details...</p>
      </div>
    );
  }
  ...
  if (!booking) {
    return (
      <div className="text-center py-12">
        <p className="text-[var(--text-main)] font-medium">Booking not found.</p>
        <button onClick={() => navigate('/validation')} className="mt-4 text-brand-primary font-bold">
          Back to List
        </button>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-10">
      <div className="flex items-center gap-3 px-4 sm:px-0">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-main)]">Validate Booking #{booking.booking_id}</h1>
          <p className="text-sm text-[var(--text-muted)]">{booking.client?.business_name || booking.client?.contact_name}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 px-4 sm:px-0">
        ...
        <div className="space-y-6">
          <div className="bg-[var(--bg-surface)] p-6 rounded-3xl border border-[var(--border-soft)] shadow-xl">
            <h2 className="font-bold text-[var(--text-main)] mb-4">Items in Booking</h2>
            ...
            {booking.items?.length === 0 ? (
              ...
              <div className="space-y-3">
                {booking.items?.map((item: any) => (
                  <div key={item.item_id} className="p-4 bg-[var(--bg-app)] rounded-2xl border border-[var(--border-soft)]">
                    <p className="font-bold text-[var(--text-main)] text-sm">{item.product_name}</p>
        ...
        <div className="bg-[var(--bg-surface)] p-6 rounded-3xl border border-[var(--border-soft)] shadow-xl h-fit">
          <h2 className="font-bold text-[var(--text-main)] mb-6 text-xl">Scan Item</h2>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <label className={cn(
                    ...
                    <span className={cn("text-sm font-bold flex-1", condition === 'good' ? "text-emerald-500" : "text-[var(--text-muted)]")}>Good</span>
                  </label>
                  <label className={cn(
                    "flex items-center gap-3 cursor-pointer p-4 rounded-xl border-2 transition-all",
