    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {showPl && (
        <div className="bg-[var(--bg-surface)] p-6 rounded-2xl border border-[var(--border-soft)] shadow-sm transition-all duration-300 space-y-6">
          ...
          {plStats?.currencies &&
            Object.entries(plStats.currencies).map(
              ([sym, stats]: [string, any]) => {
                const tRevenue = stats.total_revenue || 0;
                const tProjExp = stats.total_project_expenses || 0;
                const tGenExp = stats.total_general_expenses || 0;
                const tExp = parseFloat(tProjExp) + parseFloat(tGenExp);
                const tProfit = stats.total_profit || 0;
                const tLoss = stats.total_loss || 0;

                return (
                  <div key={sym} className="mb-6">
          ...
          <div>
            <h4 className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-widest mb-3">
              Monthly Revenue, Expenses & Profit/Loss — {plYear}
            </h4>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-[var(--bg-surface)] p-6 rounded-2xl border border-[var(--border-soft)] shadow-sm transition-all duration-300">
          <h3 className="font-bold text-[var(--text-main)] mb-6">
            {isAdmin ? "SaaS Tenants Overview" : "Recent Activity"}
          </h3>
              ...
              stats.recent_activity.map((item: any, i: number) => {
                ...
                return (
                  <div key={i} className="flex gap-4 group cursor-default">
                    <div
                      className={`p-2 rounded-xl h-fit transition-colors ${bgClass}`}
                    >
                      <Icon className={`w-5 h-5 ${colorClass}`} />
                    </div>
                    <div>
                      <p className="text-sm font-medium text-[var(--text-main)] group-hover:text-brand-primary transition-colors">
                        {item.title}
                      </p>
