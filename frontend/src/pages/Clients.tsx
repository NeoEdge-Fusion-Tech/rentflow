...
export function Clients() {
  ...
  const openEditModal = (client: any) => {
    const isStandardIndustry = [
      "tech",
      "retail",
      "finance",
      "other",
      "",
    ].includes(client.industry || "");
  ...
  const handleDelete = async (id: number) => {
    showConfirm({
      title: "Delete Client",
      message:
        "Are you sure you want to delete this client? All historic data will be preserved but the profile will be removed.",
      type: "danger",
      confirmText: "Delete",
      onConfirm: async () => {
        try {
              ...
              <div className="flex items-center justify-between pt-6 border-t border-[var(--border-subtle)]">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs font-bold text-[var(--text-main)]">
                      {client.standalone_invoices_count || 0}
                    </span>
                    <span className="text-xs text-[var(--text-muted)]">
                      Invoices
                    </span>
          ...
          <div className="md:hidden divide-y divide-[var(--border-subtle)]">
            {displayedClients.map((client) => (
              ...
              >
                ...
                <div className="flex flex-col gap-1 text-xs text-[var(--text-muted)] pl-[52px] mb-3">
                  {(client.email || client.contact_email) && (
                ...
                <div className="mt-3 pl-1 pr-1">
                  <textarea
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        tax_information: e.target.value,
                      })
                    }
            ...
            <div className="p-6 space-y-6 overflow-y-auto">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-medium text-[var(--text-main)]">
                    {viewingClient.contact_name || "N/A"}
                  </p>
                </div>
              </div>

              {(viewingClient.account_name ||
                viewingClient.bank_name ||
                viewingClient.account_number) && (
                <div>
                  <h4 className="text-xs font-bold uppercase text-[var(--text-muted)] mb-1">
                    Account Details
                  </h4>
                  <p className="text-sm font-medium text-[var(--text-main)]">
                    {viewingClient.bank_name && (
                      <span>{viewingClient.bank_name} </span>
                    )}
                    {viewingClient.account_number && (
                      <span>- {viewingClient.account_number}</span>
                    )}
                  </p>
                  {viewingClient.account_name && (
                  ...
                  {viewingClient.bank_code && (
                    <p className="text-xs text-[var(--text-muted)] mt-1">
                      Code: {viewingClient.bank_code}
                    </p>
                  )}
                </div>
              )}
