...
export function FeedbackForms() {
  ...
  const [currentForm, setCurrentForm] = useState<Partial<FeedbackForm>>({
    title: "",
    description: "",
    questions: [],
  });
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">
            {currentForm.id ? "Edit Feedback Form" : "New Feedback Form"}
          </h1>
          <div className="flex gap-2">
            <button
              onClick={() => setIsEditing(false)}
              className="px-4 py-2 border border-[var(--border-soft)] rounded-xl hover:bg-[var(--bg-app)] flex items-center gap-2"
            >
        ...
        <div className="bg-[var(--bg-surface)] p-6 rounded-2xl border border-[var(--border-soft)] space-y-4">
          <div className="flex items-center justify-between border-b border-[var(--border-soft)] pb-4 mb-4">
            ...
            <button
              onClick={addQuestion}
              className="text-sm font-medium text-brand-primary hover:bg-brand-primary/10 px-3 py-1.5 rounded-lg flex items-center gap-2"
            >
            ...
            <div className="space-y-4">
              {currentForm.questions?.map((q, idx) => (
                <div
                  ...
                  className={`flex gap-4 items-start p-4 bg-[var(--bg-app)] rounded-xl border relative group transition-all ${
                    draggedIndex === idx
                      ? "opacity-50 border-brand-primary"
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[var(--text-muted)] text-sm">
            Create and manage feedback templates for your events.
          </p>
        </div>
        <button
          onClick={() => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {forms.map((form) => (
          <div
            key={form.id}
            className="bg-[var(--bg-surface)] p-6 rounded-2xl border border-[var(--border-soft)] hover:border-brand-primary/50 transition-colors shadow-sm group"
          >
            <h3 className="font-bold text-lg mb-2 text-[var(--text-main)] truncate">
              {form.title}
            </h3>
        ...
        {forms.length === 0 && !loading && (
          <div className="col-span-full py-12 text-center border-2 border-dashed border-[var(--border-soft)] rounded-2xl">
            ...
            <p className="text-[var(--text-muted)] text-sm mb-4">
              Create your first form template to start collecting client
              feedback.
            </p>
            <button
              onClick={() => {
