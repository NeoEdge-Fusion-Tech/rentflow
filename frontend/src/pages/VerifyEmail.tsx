...
export function VerifyEmail() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.error || "Verification failed. Invalid code.",
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen flex bg-[var(--bg-app)] items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative">
      ...
      <div className="max-w-md w-full space-y-8 bg-[var(--bg-surface)] p-10 rounded-2xl shadow-sm border border-[var(--border-soft)]">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          {error && (
            <div className="text-rose-500 text-sm text-center font-medium bg-rose-500/10 p-2 rounded">
              {error}
            </div>
          )}
          {success && (
            <div className="text-emerald-500 text-sm text-center font-medium bg-emerald-500/10 p-2 rounded">
              {success}
            </div>
          )}
