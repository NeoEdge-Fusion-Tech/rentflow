...
export function PublicFeedback() {
  ...
  const fetchForm = async (id: string) => {
    try {
      ...
      res.data.form.questions?.forEach((q: FeedbackQuestion) => {
        if (q.id) {
          if (q.question_type === "TEXT" || q.question_type === "RADIO")
            initialAnswers[q.id] = "";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const formattedAnswers = Object.entries(answers)
      .map(([qId, val]) => {
        const q = projectFeedback.form.questions?.find(
        ...
        return {
          ...
          answer_text:
            q.question_type === "TEXT" || q.question_type === "RADIO"
              ? val
              : null,
    ...
    return (
      <div className="min-h-screen bg-[var(--bg-app)] flex items-center justify-center p-4">
        <div className="bg-[var(--bg-surface)] p-8 rounded-2xl max-w-md w-full text-center border border-[var(--border-soft)] shadow-xl">
          ...
          <h1 className="text-xl font-bold text-[var(--text-main)] mb-2">
            Oops!
          </h1>
      ...
      <div className="min-h-screen bg-[var(--bg-app)] flex items-center justify-center p-4">
        <div className="bg-[var(--bg-surface)] p-8 rounded-2xl max-w-md w-full text-center border border-[var(--border-soft)] shadow-xl">
          ...
          <p className="text-[var(--text-muted)]">
            Your feedback has been successfully submitted and helps us improve
            our services.
          </p>
        </div>
      </div>
      ...
      <div className="w-full max-w-2xl">
        <div className="bg-[var(--bg-surface)] p-8 rounded-t-3xl border-b-[8px] border-brand-primary shadow-sm mb-6 text-center">
          <div className="flex flex-col items-center justify-center mb-6">
            {projectFeedback.event_details?.organization_logo && (
              <img
                src={projectFeedback.event_details.organization_logo}
                alt={projectFeedback.event_details.organization_name}
                className="h-16 mb-3 object-contain"
              />
            )}
            <div className="text-xl font-black text-[var(--text-main)] opacity-90">
              {projectFeedback.event_details?.organization_name}
            </div>
          </div>

          <h1 className="text-3xl font-bold text-[var(--text-main)] mb-2">
            {projectFeedback.form.title}
          </h1>
          ...
          {projectFeedback.event_details && (
            <div className="text-sm font-medium text-[var(--text-muted)] mb-4 bg-[var(--bg-app)] inline-block px-4 py-2 rounded-xl">
              <span className="font-bold text-[var(--text-main)]">
                {projectFeedback.event_details.name}
              </span>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {projectFeedback.form.questions?.map((q, idx) => (
            <div
              key={q.id}
              className="bg-[var(--bg-surface)] p-6 md:p-8 rounded-2xl shadow-sm border border-[var(--border-soft)]"
            >
                  ...
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      ...
                      className={`transition-colors ${
                        answers[q.id!] >= star
                          ? "text-amber-400"
                          : "text-[var(--border-soft)] hover:text-amber-400/50"
                      }`}
                    >
                      ★
                    </button>
                  ...
                  <button
                    ...
                    className={`flex-1 py-3 px-4 rounded-xl border font-medium transition-colors ${
                      answers[q.id!] === true
                        ...
                        : "border-[var(--border-soft)] text-[var(--text-muted)] hover:bg-[var(--bg-app)]"
                    }`}
                  >
                    Yes
                  </button>
