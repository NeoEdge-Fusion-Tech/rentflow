...
export function Quotations() {
  ...
  const handleConvert = (quotation: any) => {
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          const res = await QuotationService.convertToInvoice(quotation.quotation_id);
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          ...
          showNotification(isTrash ? "Quotation permanently deleted" : "Quotation moved to trash", 'success');
          fetchQuotations();
        } catch (err: any) {
          console.error("Failed to delete quotation", err);
          showNotification(err.response?.data?.detail || "Failed to delete quotation", 'error');
        }
      }
    });
  };
  ...
  const handleEmptyTrash = () => {
    showConfirm({
      title: 'Empty Trash',
      message: 'Are you sure you want to permanently delete all quotations in the trash? This action cannot be undone.',
      type: 'danger',
  ...
  const displayedQuotations = quotations.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);

  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-main)]">Quotations</h1>
          <p className="text-[var(--text-muted)]">Send price quotes to prospects and convert them into invoices once accepted.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => navigate('/quotations/new')} className="flex items-center justify-center gap-2 bg-brand-primary text-brand-accent px-4 py-2.5 rounded-xl font-bold hover:opacity-90 transition-colors shadow-sm shadow-brand-primary/20">
      ...
      <div className="flex items-center gap-2 border-b border-[var(--border-soft)] pb-px overflow-x-auto">
        {['All', 'draft', 'sent', 'accepted', 'rejected', 'converted', 'Trash'].map((tab) => (
          <button
            key={tab}
      ...
      <div className="grid grid-cols-1 gap-4">
        {isLoading ? (
          <div className="bg-[var(--bg-surface)] p-12 text-center text-[var(--text-muted)] rounded-2xl border border-[var(--border-soft)]">Loading quotations...</div>
        ) : displayedQuotations.length === 0 ? (
          <div className="bg-[var(--bg-surface)] p-12 text-center text-[var(--text-muted)] rounded-2xl border border-[var(--border-soft)]">No quotations found.</div>
        ) : displayedQuotations.map((quotation) => (
          <div key={quotation.quotation_id} className="bg-[var(--bg-surface)] p-5 rounded-2xl border border-[var(--border-soft)] hover:border-brand-primary/20 hover:shadow-md transition-all group">
            <div className="flex flex-col lg:flex-row lg:items-center gap-6">
              <div className="flex-1 flex items-start gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">{quotation.quotation_number}</span>
                    <span className={cn(
                      "px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider shadow-sm border",
                      quotation.status === 'sent' ? "bg-blue-50 text-blue-700 border-blue-100" :
                      quotation.status === 'accepted' ? "bg-emerald-50 text-emerald-700 border-emerald-100" :
                      quotation.status === 'converted' ? "bg-purple-50 text-purple-700 border-purple-100" :
                      quotation.status === 'rejected' || quotation.status === 'cancelled' ? "bg-rose-50 text-rose-700 border-rose-100" :
                      "bg-[var(--bg-app)] text-[var(--text-muted)] border-[var(--border-soft)]"
                    )}>
                      {quotation.status}
                    </span>
                  </div>
                  <h3 className="text-lg font-bold text-[var(--text-main)] mt-0.5">{quotation.client_name || 'No client'}</h3>
                  <div className="flex items-center gap-4 mt-2">
                    <div className="flex items-center gap-1.5 text-xs text-[var(--text-muted)]">
                      <Calendar className="w-3.5 h-3.5" />
                      Issued {new Date(quotation.issue_date).toLocaleDateString()}
                    </div>
                    {quotation.expiry_date && (
                      <div className="flex items-center gap-1.5 text-xs text-[var(--text-muted)]">
                        Valid until {new Date(quotation.expiry_date).toLocaleDateString()}
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="text-right lg:min-w-[140px]">
                ...
                <p className="text-xl font-black text-[var(--text-main)]">{quotation.currency_symbol || defaultCurrencySymbol}{formatCurrency(quotation.total_amount)}</p>
              </div>
              ...
              <div className="flex items-center gap-2 flex-wrap border-t lg:border-t-0 lg:border-l border-[var(--border-subtle)] pt-4 lg:pt-0 lg:pl-6">
                <button
                  onClick={() => navigate(`/quotations/${quotation.quotation_id}/preview`)}
                ...
                {quotation.status !== 'converted' && (
                  <button
                    onClick={() => navigate(`/quotations/${quotation.quotation_id}/edit`)}
                ...
                <button
                  onClick={() => navigate(`/quotations/new?duplicate_from=${quotation.quotation_id}`)}
                  className="p-2.5 bg-[var(--bg-app)] text-[var(--text-main)] rounded-xl hover:bg-[var(--border-soft)] transition-colors"
                ...
                {quotation.status !== 'converted' && (
                  (quotation.status !== 'cancelled' || currentUser?.role === 'admin') && (
                    <button
                      ...
                      title={quotation.status === 'cancelled' ? "Permanently Delete" : "Delete Quotation"}
                    >
      ...
      {quotations.length > itemsPerPage && (
        <div className="flex items-center justify-between mt-4 border-t border-[var(--border-soft)] pt-4">
          <p className="text-sm text-[var(--text-muted)]">
            Showing {(currentPage - 1) * itemsPerPage + 1} to {Math.min(currentPage * itemsPerPage, quotations.length)} of {quotations.length}
          </p>
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setCurrentPage(p => Math.min(Math.ceil(quotations.length / itemsPerPage), p + 1))} className="p-2 bg-[var(--bg-surface)] border border-[var(--border-soft)] rounded-lg text-[var(--text-main)] hover:bg-[var(--bg-app)] disabled:opacity-50" disabled={currentPage === Math.ceil(quotations.length / itemsPerPage)}>
