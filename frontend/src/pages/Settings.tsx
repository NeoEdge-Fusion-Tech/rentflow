...
export function Settings() {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!myProfilePassword.current_password || !myProfilePassword.new_password)
      return;
    try {
  ...
  useEffect(() => {
    if (activeTab === "Billing & Plans" && currentUser) {
      PaymentService.getOrganizationSubscription()
        .then((res) => setSubscriptionDetails(res.data))
        .catch(console.error);
      PaymentService.getSubscriptionPayments()
        .then((res) => setSubscriptionPayments(res.data.results || res.data))
        .catch(console.error);
  ...
  const handleDeleteLogo = async () => {
    ...
    try {
      ...
      const res = await OrganizationService.patch(org.id, {
        company_logo: null,
      });
      setOrg(res.data);
    ...
    try {
      if (editingBankAccount) {
        await BankAccountService.update(
          editingBankAccount.bank_account_id,
          bankFormData,
        );
      } else {
      ...
      showNotification(
        editingBankAccount ? "Bank account updated!" : "Bank account added!",
        "success",
      );
      fetchBankAccounts();
    } catch (err) {
  ...
  const handleDeleteBankAccount = (id: number) => {
    showConfirm({
      ...
      message:
        "Are you sure you want to delete this bank account? Invoices referencing it will no longer show these payment details on download.",
      type: "danger",
      confirmText: "Delete",
      onConfirm: async () => {
        try {
  ...
  const handleRemoveUser = async (id: number) => {
    showConfirm({
      ...
      message:
        "Are you sure you want to remove this team member? They will lose all access immediately.",
      type: "danger",
      confirmText: "Remove",
      onConfirm: async () => {
        try {
  ...
  const handleSetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await UserService.adminChangePassword(selectedUser.id, {
  ...
  const handleUpgrade = async (plan_name: string) => {
    try {
      ...
      showNotification(
        e.response?.data?.error || "Failed to initialize payment",
        "error",
      );
    } finally {
    ...
    <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="space-y-6 max-w-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="bg-[var(--bg-surface)] rounded-2xl border border-[var(--border-soft)] shadow-sm p-6">
            <h2 className="text-lg font-bold text-[var(--text-main)] mb-6 flex items-center gap-2">
              <Shield className="w-5 h-5" /> My Profile
            </h2>
            <div className="mb-8">
            ...
            <div className="pt-6 border-t border-[var(--border-soft)]">
              <h3 className="text-md font-bold text-[var(--text-main)] mb-4">
                Change Password
              </h3>
              <form onSubmit={handleChangePassword} className="space-y-4">
                <div>
                  ...
                  <input
                    type="password"
                    value={myProfilePassword.current_password}
                    onChange={(e) =>
                    ...
                    required
                    className="w-full px-3 py-2 border border-[var(--border-soft)] bg-[var(--bg-app)] text-[var(--text-main)] rounded-lg focus:outline-none focus:ring-brand-primary"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-[var(--text-muted)] mb-1">
                  ...
                  <input
                    type="password"
                    value={myProfilePassword.new_password}
                    onChange={(e) =>
                      setMyProfilePassword({
                        ...myProfilePassword,
                ...
                <button
                  ...
                  className="px-4 py-2 bg-brand-primary text-white font-bold rounded-xl hover:opacity-90 disabled:opacity-50"
                >
      ...
      {activeTab === "Workspace" && (
        <div className="space-y-6 max-w-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="bg-[var(--bg-surface)] rounded-2xl border border-[var(--border-soft)] shadow-sm p-6">
            <h2 className="text-lg font-bold text-[var(--text-main)] mb-6 flex items-center gap-2">
              <Building className="w-5 h-5" /> Workspace Profile
            </h2>
            <p className="text-xs text-[var(--text-muted)] -mt-4 mb-6">
              This branding appears on every invoice and receipt you generate.
            </p>
            ...
            <div className="space-y-6">
                ...
                <div className="flex items-center gap-6">
                  <div className="w-20 h-20 bg-[var(--bg-app)] border border-[var(--border-soft)] rounded-xl flex items-center justify-center border-dashed overflow-hidden">
                    {org?.company_logo ? (
                      ...
                      <span className="text-xs text-center text-[var(--text-muted)] px-1">
                        Square Logo
                      </span>
                    )}
                  </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full px-3 py-2 border border-[var(--border-soft)] bg-[var(--bg-app)] text-[var(--text-main)] rounded-lg focus:outline-none focus:ring-brand-primary focus:border-brand-primary resize-none"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setOrg({ ...org, phone_number: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-[var(--border-soft)] bg-[var(--bg-app)] text-[var(--text-main)] rounded-lg focus:outline-none focus:ring-brand-primary focus:border-brand-primary"
                  />
                </div>
              ...
              <div>
                ...
                <select
                  value={org?.currency?.id || org?.currency_id || ""}
                  onChange={(e) => {
                    const curId = parseInt(e.target.value);
                    setOrg({
                      ...org,
                      currency_id: curId,
                      currency: currencies.find((c) => c.id === curId),
                    });
                  }}
                  className="w-full px-3 py-2 border border-[var(--border-soft)] bg-[var(--bg-app)] text-[var(--text-main)] rounded-lg focus:outline-none focus:ring-brand-primary focus:border-brand-primary"
                >
              ...
              <div>
                ...
                <p className="text-xs text-[var(--text-muted)] mt-1">
                  Used for invoices and receipts to match your brand.
                </p>
              </div>

              <button
                onClick={handleUpdateWorkspace}
                className="bg-brand-primary text-brand-accent px-6 py-2 rounded-lg font-medium hover:opacity-90 transition-opacity"
              >
                Save Changes
              </button>
            </div>
          ...
          <div className="bg-[var(--bg-surface)] rounded-2xl border border-[var(--border-soft)] shadow-sm p-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold bg-brand-primary/10 text-brand-primary border border-brand-primary/20 rounded-lg hover:bg-brand-primary/20 transition-colors"
              >
                <Plus className="w-3.5 h-3.5" /> Add Account
              </button>
            </div>
            <p className="text-xs text-[var(--text-muted)] mb-6">
              Pick which account to display when generating an invoice's payment
              details.
            </p>
              ...
              {bankAccounts.map((account) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() =>
                        handleDeleteBankAccount(account.bank_account_id)
                      }
                  ...
                  <div className="flex gap-2 ml-4">
                    ...
                    {u.id !== currentUser?.id && (
                      <button
                        ...
                        className={`text-xs px-3 py-1.5 border rounded-lg transition-colors shadow-sm font-medium ${
                          u.is_active
          ...
          <div className="bg-[var(--bg-surface)] rounded-2xl max-w-md w-full shadow-xl border border-[var(--border-soft)]">
            ...
            <form onSubmit={handleAddUser} className="p-6 space-y-4">
              ...
              <div className="pt-4 flex justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="px-5 py-2.5 bg-brand-primary text-brand-accent font-medium rounded-xl hover:opacity-90 transition-opacity shadow-sm shadow-brand-primary/20"
                >
                  Add Member
                </button>
              </div>
          ...
          <div className="bg-[var(--bg-surface)] rounded-2xl max-w-md w-full shadow-xl overflow-hidden border border-[var(--border-soft)]">
            <div className="flex items-center justify-between p-6 border-b border-[var(--border-soft)] bg-[var(--bg-app)]/50">
              <div>
                ...
                <p className="text-xs text-[var(--text-muted)]">
                  Updating password for: {selectedUser?.email}
                </p>
              </div>
              <button
                onClick={() => {
            ...
            <form onSubmit={handleSaveBankAccount} className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full border border-[var(--border-soft)] rounded-xl p-2.5 outline-none focus:border-brand-primary bg-[var(--bg-app)] text-[var(--text-main)]"
                >
                  <option value="savings" className="bg-[var(--bg-surface)]">
                    Savings
                  </option>
                  <option value="current" className="bg-[var(--bg-surface)]">
                    Current
                  </option>
                </select>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full border border-[var(--border-soft)] rounded-xl p-2.5 outline-none focus:border-brand-primary bg-[var(--bg-app)] text-[var(--text-main)] resize-none"
                />
              </div>
          ...
          <div className="bg-[var(--bg-surface)] rounded-2xl w-full max-w-md overflow-hidden shadow-2xl border border-[var(--border-soft)] flex flex-col">
            ...
            <div className="p-4 flex justify-between items-center gap-4 bg-[var(--bg-surface)]">
              ...
              <button
                ...
                className="px-5 py-2 bg-brand-primary text-brand-accent font-medium rounded-xl hover:opacity-90 transition-opacity whitespace-nowrap"
              >
